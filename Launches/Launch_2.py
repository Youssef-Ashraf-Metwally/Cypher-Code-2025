...
def Mission():
    ...
    Header.Left_Motor_M.run_target(700, 1750, then=Stop.HOLD, wait=False)
    Header.PID(-60, 1, 200, -5, -0.1, -10, 15)
    Header.Right_Motor_M.run_target(1000, -800, then=Stop.HOLD)
    Header.wait(500)
    Header.Right_Motor_M.run_target(1400, 1500, then=Stop.HOLD)
    Header.Right_Motor_M.run_target(600, 400, then=Stop.HOLD)
