...
def Mission():
    ...
    Header.PID(500, 0, 450, -5, -0.1, -10, 15)
    Header.Left_Motor_M.run_target(-1000, -300, then=Stop.HOLD, wait=True)
    Header.Right_Motor_M.run_time(1600, 950, then=Stop.HOLD, wait=False)
    Header.P_Gyro_Turn(-23, 10, 10)
    Header.PID(-500, -23, 230, -5, -0.1, -10, 15)
    Header.Right_Motor_M.reset_angle(0)
    Header.Right_Motor_M.run_target(-1600, -300, then=Stop.HOLD, wait=True)
    Header.PID(-500, -23, 350, -5, -0.1, -10, 15)
    Header.P_Gyro_Turn(0, 10, 10)
    Header.PID(-500, 0, 450, -5, -0.1, -10, 15)
    Header.Left_Motor_M.run_target(1000, 200, then=Stop.HOLD, wait=False)
    Header.P_Gyro_Turn(45, 10, 10)
    Header.PID(500, 45, 270, -5, -0.1, -10, 15)
    Header.PID(-450, 25, 700, -3, -0.1, -10, 15)
    Header.Robot_Break()
